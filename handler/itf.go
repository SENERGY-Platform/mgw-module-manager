...
type DepStorageHandler interface {
	...
	ListDepReq(ctx context.Context, dID string) ([]model.DepMeta, error)
	ListDepReqBy(ctx context.Context, dID string) ([]model.DepMeta, error)
	...
	UpdateDep(ctx context.Context, tx driver.Tx, dID, name string, indirect bool, timestamp time.Time) error
