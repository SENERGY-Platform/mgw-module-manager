...
func (h *Handler) Get(ctx context.Context, mID string) (lib_model.Module, error) {
	return h.storageHandler.Get(ctx, mID)
}

func (h *Handler) GetReq(ctx context.Context, mID string) (lib_model.Module, map[string]lib_model.Module, error) {
	m, err := h.storageHandler.Get(ctx, mID)
	if err != nil {
		return lib_model.Module{}, nil, err
	}
	dep := make(map[string]lib_model.Module)
...
func (h *Handler) Add(ctx context.Context, mod *module.Module, modDir dir_fs.DirFS, modFile string, indirect bool) error {
	...
	m := lib_model.Module{
		Module: mod,
		ModuleExtra: lib_model.ModuleExtra{
			Indirect: indirect,
			Added:    t,
			Updated:  t,
		},
...
func (h *Handler) Delete(ctx context.Context, mID string, force bool) error {
	...
	if !force {
		l, err := h.storageHandler.List(ctx, lib_model.ModFilter{InDependencies: map[string]struct{}{mID: {}}})
		if err != nil {
			return err
		}
		var ids []string
		...
		return lib_model.NewInternalError(fmt.Errorf("module is required by: %s", strings.Join(ids, ", ")))
	}
	ch := context_hdl.New()
	defer ch.CancelAll()
...
func (h *Handler) getReqMod(ctx context.Context, mod *module.Module, reqMod map[string]lib_model.Module) error {
