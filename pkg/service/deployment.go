...
func (s *Service) DeploymentRequest(ctx context.Context, moduleIds []string) ([]models_service2.Module, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	...
	var modules []models_service2.Module
...
func (s *Service) CreateDeployments(ctx context.Context, userInputs []models_service2.UserInput) (models_service2.Job, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	handlerModules, err := s.modulesHandler.Modules(ctx, models_handler_modules.ModuleFilter{
		Ids: helper_slices.CollectFunc(slices.Values(userInputs), func(item models_service2.UserInput) string {
	...
	go func() {
		defer job.Done()
		jobResult := models_service2.JobResultDeployments{
...
func (s *Service) UpdateDeployments(ctx context.Context, userInputs []models_service2.UserInput) (models_service2.Job, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	currentJobs := s.jobsHandler.CurrentSlotJobs([]int{deploymentJobSlotNum, moduleJobSlotNum})
	if len(currentJobs) > 0 {
	...
	go func() {
		defer job.Done()
		jobResult := models_service2.JobResultUpdateDeployments{
			JobResult: models_service2.JobResult{JobId: job.Id},
		}
		defer func() {
			if err := recover(); err != nil {
		...
		for _, updateDepResult := range updateDepResults {
			result := models_service2.JobResultUpdateDeploymentsResult{Result: updateDepResult}
			if !updateDepResult.HasError {
...
func (s *Service) RecreateDeployments(ctx context.Context, moduleIds []string) (models_service2.Job, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	currentJobs := s.jobsHandler.CurrentSlotJobs([]int{deploymentJobSlotNum, moduleJobSlotNum})
	if len(currentJobs) > 0 {
...
func (s *Service) DeleteDeployments(ctx context.Context, moduleIds []string) ([]models_service2.DeleteDeploymentsResult, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	auxResults := make(map[string]models_service2.DeleteAuxiliaryDeploymentResult)
	var toDelete []string
	for id := range deploymentIds {
		var auxResult models_service2.DeleteAuxiliaryDeploymentResult
	...
	var results []models_service2.DeleteDeploymentsResult
	for id, moduleId := range deploymentIds {
		...
		results = append(results, models_service2.DeleteDeploymentsResult{
...
func getUserInputs(
	userInputs []models_service2.UserInput,
