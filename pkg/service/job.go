...
func (s *Service) Jobs(_ context.Context, filterIds []string) ([]lib_service.Job, error) {
	handlerJobs := s.jobsHandler.Jobs(filterIds)
	var jobs []lib_service.Job
	for _, handlerJob := range handlerJobs {
		jobs = append(jobs, getJob(handlerJob))
	}
	slices.SortStableFunc(jobs, func(a, b lib_service.Job) int {
		return a.Start.Compare(b.Start)
	})
	return jobs, nil
}

func (s *Service) Job(_ context.Context, jobId string) (lib_service.Job, error) {
...
func getJob(handlerJob *handler_jobs.Job) lib_service.Job {
	job := lib_service.Job{
		Id:          handlerJob.Id,
