...
func (s *Service) GetGlobalConfig(ctx context.Context, id string) (lib_models.GlobalConfig, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *Service) GetGlobalConfigs(ctx context.Context, ids []string) (map[string]lib_models.GlobalConfig, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	...
	globalConfigs := make(map[string]lib_models.GlobalConfig)
...
func (s *Service) UpdateGlobalConfig(ctx context.Context, config lib_models.GlobalConfig) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func newGlobalConfig(config pkg_models.Config) lib_models.GlobalConfig {
	return lib_models.GlobalConfig{
		Id:   config.Id,
		Name: config.Name,
		InterfaceValue: lib_models.InterfaceValue{
