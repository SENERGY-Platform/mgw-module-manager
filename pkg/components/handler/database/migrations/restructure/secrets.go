...
func migrateSecretsTab(ctx context.Context, db *sql.DB) error {
	...
	ok, err = indexExists(ctx, db, tableName, "uk_dep_id_ref_item")
	if err != nil {
		return err
	}
	if !ok {
		logger.InfoContext(ctx, "adding unique index", attrIndex, "uk_dep_id_ref_item", attrTable, tableName)
		err = addUniqueIndex(ctx, db, tableName, "uk_dep_id_ref_item", "dep_id", "ref", "item")
		if err != nil {
			return err
		}
	}
	...
	newIndexKeys := []string{"uk_dep_id_ref_item", "i_dep_id"}
	...
	logger.InfoContext(ctx, "renaming table", attrTable, tableName, attrNewName, "dep_secrets")
	return renameTable(ctx, db, tableName, "dep_secrets")
}
