...
func (h *Handler) runtimeMonitorJobsFilter(
	auxDepsByParent map[string]models_handler_database.AuxiliaryDeploymentParent,
) map[string]models_handler_database.AuxiliaryDeploymentParent {
	h.runtimeMonitorJobsMu.RLock()
	defer h.runtimeMonitorJobsMu.RUnlock()
	filteredDeployments := make(map[string]models_handler_database.AuxiliaryDeploymentParent)
