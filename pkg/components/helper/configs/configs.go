...
func configToString(config models_handler_database.Config) string {
...
func sliceConfigToString(config models_handler_database.Config, delimiter string) string {
	var values []string
	switch config.DataType {
	case models_handler_database.StringType:
...
func ConfigIsEqual(a, b models_handler_database.Config) bool {
...
func GetConfig(val any, moduleConfig models_external.ModuleLibConfigValue) (models_handler_database.Config, error) {
	...
	if moduleConfig.IsSlice {
		anySlice, ok := val.([]any)
		if !ok {
			return models_handler_database.Config{}, fmt.Errorf("invalid data type '%T'", val) // TODO
		}
		switch moduleConfig.DataType {
		case models_external.ModuleLibStringType:
			config.DataType = models_handler_database.StringType
			for _, item := range anySlice {
				v, err := toString(item)
				if err != nil {
					return models_handler_database.Config{}, err
				}
		...
		case models_external.ModuleLibBoolType:
			config.DataType = models_handler_database.BoolType
			for _, item := range anySlice {
				v, err := toBool(item)
				if err != nil {
					return models_handler_database.Config{}, err
		...
		case models_external.ModuleLibInt64Type:
			config.DataType = models_handler_database.Int64Type
			for _, item := range anySlice {
				v, err := toInt64(item)
				if err != nil {
					return models_handler_database.Config{}, err
		...
		case models_external.ModuleLibFloat64Type:
			config.DataType = models_handler_database.Float64Type
			for _, item := range anySlice {
		...
		default:
			return models_handler_database.Config{}, fmt.Errorf("unknown data type '%s'", moduleConfig.DataType) // TODO
		}
	} else {
		switch moduleConfig.DataType {
		case models_external.ModuleLibStringType:
