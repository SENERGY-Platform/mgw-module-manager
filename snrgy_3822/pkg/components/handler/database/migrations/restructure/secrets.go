...
func migrateSecretsTab(ctx context.Context, db *sql.DB) error {
	...
	if !ok {
		logger.Info("adding unique index", attrIndex, "uk_dep_id_ref_item", attrTable, tableName)
	...
	logger.Info("renaming table", attrTable, tableName, attrNewName, "dep_secrets")
