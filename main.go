...
func main() {
	...
	if err != nil {
		logger.Error("creating mysql connector failed", attr_keys.Error, err)
		ec = 1
		return
	}
	...
	if err != nil {
		logger.Error("database migration failed", attr_keys.Error, err)
		ec = 1
		return
	}
	...
	if err != nil {
		logger.Error("creating http engine failed", attr_keys.Error, err)
		ec = 1
		return
	}
	...
	if err != nil {
		logger.Error("creating server listener failed", attr_keys.Error, err)
		ec = 1
		return
	}
	...
	if err != nil {
		logger.Error("initializing module repositories failed", attr_keys.Error, err)
		ec = 1
		return
	}
	...
	err = modulesHdl.Init()
	if err != nil {
		logger.Error("initializing modules handler failed", attr_keys.Error, err)
		ec = 1
		return
	}
	...
	if err != nil {
		logger.Error("initializing deployments handler failed", attr_keys.Error, err)
		ec = 1
		return
	}
	...
	go func() {
		...
		if err := httpServer.Serve(serverListener); !errors.Is(err, http.ErrServerClosed) {
			logger.Error("starting server failed", attr_keys.Error, err)
			ec = 1
		}
		cf()
	}()
	...
	go func() {
		...
		if err := httpServer.Shutdown(ctxWt); err != nil {
			logger.Error("stopping server failed", attr_keys.Error, err)
			ec = 1
		} else {
