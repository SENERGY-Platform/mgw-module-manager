...
type ModStagingHandler interface {
	Prepare(ctx context.Context, modules map[string]*module.Module, mID, ver string) (model.Stage, error)
}
...
type ModUpdateHandler interface {
	...
	Prepare(ctx context.Context, modules map[string]*module.Module, stage model.Stage, mID string) error
